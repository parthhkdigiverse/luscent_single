...
export const ContactPage = () => {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_URL}/api/content/contact_info`, { cache: 'no-store' }).then(res => res.ok ? res.json() : null),
      fetch(`${API_URL}/api/settings/public`, { cache: 'no-store' }).then(res => res.ok ? res.json() : null)
    ])
      .then(([contactData, settingsData]) => {
        setSettings(prev => ({
          ...prev,
          ...(contactData && contactData.content ? {
            contact_email: contactData.content.email || prev.contact_email,
            contact_phone: contactData.content.phone || prev.contact_phone,
            contact_address: contactData.content.address || prev.contact_address,
          } : {}),
          ...(settingsData && !settingsData.detail ? {
        ...
        <div className="lg:col-span-5 bg-white border border-brand-card/50 shadow-sm rounded-3xl p-6 md:p-8 space-y-8">
          ...
          <div className="pt-6 border-t border-brand-card/50">
            ...
            <div className="flex gap-3">
              {settings.instagram && (
                <a href={settings.instagram} target="_blank" rel="noreferrer" className="w-9 h-9 rounded-full bg-brand-bg flex items-center justify-center text-brand-dark hover:bg-brand-dark hover:text-white transition-all">
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
                </a>
              )}
              {settings.facebook && (
                <a href={settings.facebook} target="_blank" rel="noreferrer" className="w-9 h-9 rounded-full bg-brand-bg flex items-center justify-center text-brand-dark hover:bg-brand-dark hover:text-white transition-all">
              ...
              {settings.twitter && (
                <a href={settings.twitter} target="_blank" rel="noreferrer" className="w-9 h-9 rounded-full bg-brand-bg flex items-center justify-center text-brand-dark hover:bg-brand-dark hover:text-white transition-all">
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 3a10.9 10.9 0 0 1-3.14 1.53 4.48 4.48 0 0 0-7.86 3v1A10.66 10.66 0 0 1 3 4s-4 9 5 13a11.64 11.64 0 0 1-7 2c9 5 20 0 20-11.5a4.5 4.5 0 0 0-.08-.83A7.72 7.72 0 0 0 23 3z"></path></svg>
                </a>
              )}
              {settings.youtube && (
                <a href={settings.youtube} target="_blank" rel="noreferrer" className="w-9 h-9 rounded-full bg-brand-bg flex items-center justify-center text-brand-dark hover:bg-brand-dark hover:text-white transition-all">
